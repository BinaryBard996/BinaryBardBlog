...
export function SearchPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-[#e8e4dc] mb-2 font-serif">
          搜索文章
        </h1>
      ...
      <div className="relative mb-8">
        ...
        <input
          ...
          placeholder="搜索..."
          className="w-full pl-10 h-12 text-base rounded-lg anime-panel text-[#e8e4dc] placeholder:text-[#6b6773] outline-none focus:border-anime-gold/40 transition-colors"
          autoFocus
        />
      </div>
      ...
      {/* Results */}
      {!isReady && (
        <p className="text-center text-xs text-anime-gold py-8 tracking-wider">加载中...</p>
      )}

      {isReady && query && results.length === 0 && (
        <div className="text-center py-12">
          <p className="text-[#6b6773]">
      ...
      {results.length > 0 && (
        <div className="space-y-3">
          <p className="text-xs text-[#6b6773] mb-4 tracking-wider">
          ...
          {results.map((result) => (
            <Link
              ...
              className="group flex items-start gap-3 p-4 anime-panel anime-card"
            >
              <FileText className="w-5 h-5 text-anime-gold/60 mt-0.5 flex-shrink-0" />
              <div className="flex-1 min-w-0">
                <h3
                  className="text-base font-bold text-[#e8e4dc] group-hover:text-anime-gold transition-colors"
                ...
                {result.matches.length > 0 && (
                  <p
                    className="text-sm text-[#6b6773] mt-2 line-clamp-2"
              ...
              <ArrowRight className="w-4 h-4 text-[#6b6773] mt-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0" />
            </Link>
          ))}
        </div>
      )}
